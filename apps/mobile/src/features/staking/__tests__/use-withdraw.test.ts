...
beforeEach(() => {
  ...
  jest
    .mocked(useLockStore)
    .mockReturnValue({ optimisticWithdraw: mockOptimisticWithdraw } as never);
...
describe('useWithdraw', () => {
  ...
  describe('disclaimer 미동의', () => {
    it('disclaimerAccepted가 false이면 needsDisclaimer가 true가 된다', async () => {
      const { result } = renderHook(() => useWithdraw());

      await act(async () => {
        await result.current.withdraw(TEST_KEY_ID, false);
      });
