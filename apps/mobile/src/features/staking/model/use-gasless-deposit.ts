...
import { type Address, encodeFunctionData, hashMessage } from 'viem';
import { privateKeyToAccount } from 'viem/accounts';

import type { TxState } from './staking-utils';
import { clearPendingTx, mapContractError, savePendingTx, scheduleRefetch } from './staking-utils';
...
export function useGaslessDeposit() {
  ...
  const gaslessDeposit = async (amountWei: bigint, unlockTime: bigint, keyId: string) => {
    if (isSubmitting.current || !address) {
      return;
    }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          setTxState('success');
        } catch { /* receipt timeout - pending tx stored in MMKV */ }
      } finally {
        privateKey = null;
      }
    } catch (error) {
